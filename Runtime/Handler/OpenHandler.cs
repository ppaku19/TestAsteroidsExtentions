...
namespace UnityEngine.UI
{
    public class OpenHandler : MonoBehaviour
    {
        ...
        public SimpleTween.TweenBase[] simpleTweenOpen;
        public SimpleTween.TweenBase[] simpleTweenClose;
        ...
        private void Awake()
        {
            ...
            foreach (var tween in simpleTweenOpen)
            {
                if (tween == null) continue;
                tween.Stop();
            }
            foreach (var tween in simpleTweenClose)
            {
                if (tween == null) continue;
                tween.Stop();
            }
        ...
        private IEnumerator WaitEndRoutine()
        {
            ...
            switch (state)
            {
                case State.Opening:
                    ...
                    {
                        if (tween == null) continue;
                        if (tween.loop <= 0) continue;
                        if (tween.ignoreTimeScale)
                            unscaledDuration = Mathf.Max(unscaledDuration, tween.startDelay + tween.duration * tween.loop);
                        else
                            duration = Mathf.Max(unscaledDuration, tween.startDelay + tween.duration * tween.loop);
                    }
#if DOTWEEN
                    foreach (var tweenAnim in doTweenAnimOpen)
                    {
