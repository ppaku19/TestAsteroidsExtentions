...
namespace UnityEngine.UI
{
    ...
    public class OpenHandlerEditor : Editor
    {
        ...
        private SerializedProperty m_SimpleTweenOpen;
        private SerializedProperty m_SimpleTweenClose;
#if DOTWEEN
        ...
        private void OnEnable()
        {
            ...
            m_SimpleTweenOpen = serializedObject.FindProperty("simpleTweenOpen");
            m_SimpleTweenClose = serializedObject.FindProperty("simpleTweenClose");
#if DOTWEEN
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.Space();
            EditorGUILayout.LabelField("Simple Tweens", EditorStyles.boldLabel);
            EditorGUI.indentLevel++;
            EditorGUILayout.PropertyField(m_SimpleTweenOpen, new GUIContent("Open"));
            EditorGUILayout.PropertyField(m_SimpleTweenClose, new GUIContent("Close"));
            EditorGUI.indentLevel--;
#if DOTWEEN
            EditorGUILayout.Space();
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                if (GUILayout.Button("Open"))
                    for (int i = 0; i < targets.Length; i++)
                        ((OpenHandler)targets[i]).Open();
                if (GUILayout.Button("Close"))
                    for (int i = 0; i < targets.Length; i++)
                        ((OpenHandler)targets[i]).Close();
            }
        }
    }
