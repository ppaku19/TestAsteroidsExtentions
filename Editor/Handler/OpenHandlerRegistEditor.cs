...
namespace UnityEngine.UI
{
    public class OpenHandlerRegistEditor : EditorWindow
    {
        ...
        public bool showOnlyAnims = false;
        ...
        private void OnGUI_HierarchyRecusively(Transform transform, string hierarchiedName, int depth)
        {
            ...
            if (hasAnim == false)
                hasAnim = System.Array.Exists(components, _ => _ is SimpleTween.TweenBase);
#if DOTWEEN
            if (hasAnim == false)
        ...
        private void OnGUI_Inspector()
        {
            ...
            for (int i = 0; i < m_CachedComponents.Length; i ++)
            {
                ...
                else if (component is SimpleTween.TweenBase simpleTween)
                {
                    EditorGUILayout.BeginHorizontal();
                    if (System.Array.Exists(target.simpleTweenOpen, _ => _ == simpleTween))
                    {
                        if (GUILayout.Button("Unregist Open"))
                        {
                            var list = new List<SimpleTween.TweenBase>(target.simpleTweenOpen);
                            list.Remove(simpleTween);
                            target.simpleTweenOpen = list.ToArray();
                        }
                    }
                    else
                    {
                        ...
                        {
                            var list = new List<SimpleTween.TweenBase>(target.simpleTweenOpen);
                            list.Add(simpleTween);
                            target.simpleTweenOpen = list.ToArray();
                        }
                    }
                    if (System.Array.Exists(target.simpleTweenClose, _ => _ == simpleTween))
                    {
                        if (GUILayout.Button("Unregist Close"))
                        {
                            var list = new List<SimpleTween.TweenBase>(target.simpleTweenClose);
                            list.Remove(simpleTween);
                            target.simpleTweenClose = list.ToArray();
                        }
                    }
                    else
                    {
